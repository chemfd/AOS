...
\hypertarget{ch:fluorother}{%
\chapter{Fluorescence Anisotropy \& Applications of Fluorescence}\label{ch:fluorother}}

\hypertarget{fluorescence-anisotropy}{%
\section{Fluorescence anisotropy}\label{fluorescence-anisotropy}}
...
{\centering \includegraphics[width=0.8\linewidth]{images/anisotropy} 

}

\caption{A standard or lifetime fluorimeter may be modified by the addition of two two-position polarising filters -  incident light is polarised in eithter a vertical (V) or horizontal (H) direction, excident light is then measured in each of these two polarisations.}\label{fig:anisotropy}
\end{figure}
...
The term \(G\) in equation \eqref{eq:anisotropy} is a response factor for the instrument which normalises the detector for any differences in response against the sensitivity of the detector for light polarised in the horizontal and vertical directions. Instruments do not return individual intensities of polarisation, but instead just report values of \(r\) directly.

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{images/transdipoles} 

}

\caption{The chromaphore rhoadamine 6G has a small offset in the transition dipole omoment of HOMO-LUMO absorbance and fluorescence emisison..}\label{fig:transdipoles}
\end{figure}

We have seen previously that each of the absorbance transitions (from S\textsubscript{0} to S\textsubscript{1}, S\textsubscript{2}, S\textsubscript{3}\ldots) occurs at a different angle (relative to the principle axis in the molecule), and that Kasha's rule states that all emission occurs from the vibrationally relaxed lowest excited state of that spin multiplicity (or there is just a single emission transition).
...
\label{eq:anisotropyeqn}
\end{equation}

\begin{longtable}[]{@{}cc@{}}
\caption{\label{tab:anisotropyangle} The measured anisotropy for a selectrion of different angles between the transition dipole moments of excitation and emission, (\(\beta\))}\tabularnewline
\toprule
\(\beta\) / \(^\circ\) & \(r_0\)\tabularnewline
\midrule
\endfirsthead
\toprule
\(\beta\) / \(^\circ\) & \(r_0\)\tabularnewline
\midrule
\endhead
0 & 0.40\tabularnewline
45 & 0.10\tabularnewline
54.7 & 0.00\tabularnewline
90 & -0.20\tabularnewline
\bottomrule
\end{longtable}

\hypertarget{time-resolved-anisotropy}{%
\subsection{Time-resolved anisotropy}\label{time-resolved-anisotropy}}
...
\begin{equation}
\theta = \frac{\eta V}{RT}
\label{eq:correlationtime}
\end{equation}

Equation \eqref{eq:correlationtime} has been simplified for a spherical molecule, but with small changes to this equation it it possible to model for non-spherical systems.
...
\hypertarget{application-of-fluorescence}{%
\section{Application of fluorescence}\label{application-of-fluorescence}}
...
{\centering \includegraphics[width=0.8\linewidth]{images/emissionenhancement} 

}

\caption{The enhancement of emisison when binding to a second molecule may be used to determine the thermodynamics of binding, and the number of binding sites in the system.}\label{fig:emissionenhancement}
\end{figure}

If we consider the emission at a single wavelength then we can combine information about the emission of unbound dye (\(I_0\)), the emission when all dye is bound (which is estimated from an asymptotic model, \(I_b\)) with the measured emission at that point (\(I_x\)) to determine the proportion of dye in complex (equation \eqref{eq:binding}, figure \ref{fig:enhancement}).
...
{\centering \includegraphics[width=0.8\linewidth]{images/enhancement} 

}

\caption{At a fixed wavelength if we plot the emission intensity against P/D ratio the emission intensity increases as the proportion of bound dye increases, this reaches a maximum when all dye is bound to the DNA.}\label{fig:enhancement}
\end{figure}
